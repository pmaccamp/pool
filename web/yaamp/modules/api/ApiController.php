...
class ApiController extends CommonController
{
    ...
    public function actionDifficulty()
    {
        ...
        if (!$whitelisted && !LimitRequest('api-difficulty', 10)) {
            return;
        }

        $json = controller()->memcache->get("api_difficulty");
        if (empty($json)) {
            $data  = array();
            $coins = getdbolist('db_coins', "enable AND visible AND auto_ready AND IFNULL(algo,'PoS')!='PoS' ORDER BY symbol");
            ...
            foreach ($coins as $coin) {
                ...
                $difficulty = arraySafeVal($mininginfo,"difficulty",-1);
            ...
            $json = json_encode($data);
            controller()->memcache->set("api_difficulty", $json, 1, MEMCACHE_COMPRESSED);
        }
